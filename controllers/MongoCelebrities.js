...
const getCelebrityByProfession = async (req, res = response) => {
    ...
    try {
        ...
        res.json({ Ok: true, resp: celebrity, msg: 'Celebridad obtenida por su profesion.' });
    } catch (error) {
        res.json({ Ok: false, resp: error, msg: 'Error al obtener la celebridad por su profesion.' });
    }
};
...
const createCelebrity = async (req, res = response) => {
    ...
    try {
        ...
        res.json({ Ok: false, msg:'Error al Insertar la Celebridad', resp: error });
    }
};
